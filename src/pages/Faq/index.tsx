...
const Faq = () => {
    ...
    const [openAccordion, setOpenAccordion] = useState<boolean[]>([true, true, true, true])
    ...
    return (
        <main className='faq'>
            ...
            <ul className="faq__items">
                {faqText.map((e, i) => <li key={i} className="faq-item">
                <div className="faq-item__head" onClick={()=> accordionOpen(i)}>
                    <h2 className="faq-item__title">{e.name}</h2>
                    <img src="./img/arrow.svg" alt="arrow" className={`faq-item__svg ${openAccordion[i]? 'svg-open': ''}` }/>
                </div>
                <p className={`faq-item__text ${openAccordion[i]? 'text-open': ''}` }>{e.text}</p> 
                </li>)}
            </ul>
        </main>
    );
