...
import CalkulatorBasket from '../../../component/calkulator/calkulatorBasket'
import Cashback from '../../../component/UI/cashback'
...
const BasketItems = ({BasketDB}:IProps) => {
    ...
    useEffect(()=>{
        function changeElement(){
            ...
            dispatch(setBasketDB(BasketDB.map((e, i) => i === amountIndex.index ? newOb : e)))
        };
        changeElement()
    }, [amountIndex]);
		...
		<ul className='basket-items'>
            {
                BasketDB.map((e, i)=> <li key={i} className='basket-items__item'>
                    ...
                    <CalkulatorBasket index={i} setAmountIndex={setAmountIndex} type={amountIndex.type} amountMassive={e.amount}/>
                    <Cashback num={e.cashback}/>
                    <h4 className='basket-items__price'>{e.price}$</h4>
                    <button className='basket-items__btn' onClick={()=> deletElements(i)}>
                        <svg
                            width='24'
                            height='24'
                            viewBox='0 0 24 24'
